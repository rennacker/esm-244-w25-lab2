...
y = c(12,23,34,45,56,67,78)

z = rnorm(10000)
p = rnorm(10000) # random number generation
...
```{r}
test_df <- penguins_fold |> filter(group==1)

train_df <- penguins_fold |> filter(group!=1)
```
...
rmse_predict_test<-predict_test |> 
  summarize(rmse_mdl1 = calc_rmse(model1,mass), rmse_mdl2 = calc_rmse(model2,mass),rmse_mdl3 = calc_rmse(model3,mass))
...
rmse_vec<-vector(mode = 'numeric', length=folds)  #Why? if we dont tell r to save it r will try and 
#make a vector
...
for( i in 1:folds){
  ...
  training_k_lm1 <- lm(f1, data=kfold_train_df)
         
  #Get the predictions from the model

    predict_k_test<-kfold_test_df |> 
  mutate(model1 = predict(training_k_lm1,kfold_test_df))
         
  # Summarize/calculate the rmse for that model
  kfold_rmse<-predict_k_test |> 
...
#for loop is equivalant across all coding, but purr is set up for r alone
